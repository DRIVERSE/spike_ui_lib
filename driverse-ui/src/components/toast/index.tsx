/**
 * @extracted-from
 *   A: Driverse_FE_Autocredit @ b96eda3 src/components/toast/index.tsx
 *   B: Driverse_FE_Business   @ b96eda3 src/components/toast/index.tsx
 * @status decoupled
 * @notes Byte-identical in both apps. The single decoupling is the mode source: `useSettings().themeMode`
 *        from the zustand store becomes the library's `useTheme().mode`, which is exactly the swap the
 *        plan called for. Everything else — sonner options, the five Iconify status icons, the
 *        styled-components wrapper keyed on themeVars — is verbatim.
 *        `sonner`'s `toast()` is re-exported so apps import both halves from one place, and so the W4
 *        hooks that take a `notify` callback have an obvious default to pass.
