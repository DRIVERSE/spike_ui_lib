/**
 * @extracted-from
 *   A: Driverse_FE_Autocredit @ b96eda3 src/components/upload/utils.ts
 *   B: Driverse_FE_Business   @ b96eda3 src/components/upload/utils.ts
 * @status adopted-B
 * @notes B adopted: A's copy differs only in that B translated the zh-CN JSDoc summaries. Keeps its direct sonner toast calls for the two avatar validation failures, matching the origin — sonner is a peer dependency because of this file and the Toast component.
 */
...
export function fileTypeByName(fileName = "") {
	// Lower-cased: the format tables are all lower-case, so "photo.PNG" fell through to the generic
	// file icon in both apps. One-word fix; see the origin header.
	return fileName?.split(".").pop()?.toLowerCase() || "folder";
}

export function formatDate(date: string | null, locale = "en-US") {
	if (!date) return "—";
